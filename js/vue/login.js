  ...
  methods: {
    ...
    onClickLoginBtn: function (e) {
      ...
      if(this.email == '') { // EMPTY EMAIL
        ...
        if(e.currentTarget.getAttribute('data-login-type') === 'customer') {
          this.loginUser(params, loginCustomerEndPoint);
        }
        else if(e.currentTarget.getAttribute('data-login-type') === 'employee') {
          this.loginUser(params, loginEndPoint);
        }
    ...
    loginUser: async function (params, loginEndPointOfSelectedType){
      const result = await action(baseUrl, loginEndPointOfSelectedType, params)
      const res = await result.json()
      
      // HIDE LOADING ICON
      this.removeloginBtns = false; 
      this.loading = true;

// VIEW ALL USERS
const viewAllCustomersAddedByEmployeeEndPoint = "/view_customers_by_employee";

// UPDATE USER STATUS 
const updateUserStatusEndPoint = "/update_user_status";

// ADD CUSTOMER IN USERS
const addCustomerByEmployeeInUserEndPoint = "/add_customer_by_employee";
...
new Vue({
  el: '#customersAddedByEmployeeVueApp',
  data: {
    employeeName: '',
    error: '',
  ...
  mounted: function () { // WHEN VUE COMPONENT IS ADDED TO DOM, THIS HOOK IS CALLED
    this.employeeName = this.$session.get('data')[0].name;

    this.viewAllCustomersAddedByEmployee(); // VIEW ALL CUSTOMERS

    this.viewAllActiveCountries(); // VIEW ALL ACTIVE COUNTRIES
    this.viewAllActiveRoles(); // VIEW ALL ACTIVE ROLES

    mobileNumberVerification('customer_mobile'); // VERIFY CUSTOMER MOBILE NUMBER
  },
  methods: {
    /*-------------------------------------------------------------*/
    /*-------------- CUSTOMERS ADDED BY EMPLOYEE ------------------*/
    /*-------------------------------------------------------------*/

    viewAllCustomersAddedByEmployee: async function () {
      ...
      // PARAMETERS TO VIEW ALL CUSTOMERS ADDED BY EMPLOYEE
      params = { }; 
      // GET ALL CUSTOMERS ADDED BY EMPLOYEE FROM API CALL
      const result = await actionAPICall(baseUrl, viewAllCustomersAddedByEmployeeEndPoint, params);
    ...
    onClickAddCustomerBtn: function () {
      if(this.customerName === '') { // EMPTY CUSTOMER NAME
        ...
        // ADD CUSTOMER
        this.addCustomerByEmployee(params);
      }
    },
    addCustomerByEmployee: async function (params) {
      ...
      // NO DUPLICATE EMAIL/ MOBILE NUMBER
        ...
        // ADD CUSTOMER BY EMPLOYEE ON SERVER
        const result = await actionAPICall(baseUrl, addCustomerByEmployeeInUserEndPoint, params);
        const res = await result.json();

        // CUSTOMER ADDED SUCCESSFULLY
        if(res.result === 'success') {
          // CLOSE MODAL
          closeModal('addCustomerByEmployee');

          // ENABLE ADD CUSTOMER BUTTON
          addCustomerModalBtn.disabled = false;
          
          toastr.success('Customer added successfully');
          this.viewAllCustomersAddedByEmployee(); // RELOAD ALL CUSTOMERS VIEW
    ...
    updateEmployeeCustomerStatus: async function (params) {
      ...
      if(res.result === 'success') {
        ...
        this.viewAllCustomersAddedByEmployee();
      }
      else {
        toastr.error('There was an error!');
      }
